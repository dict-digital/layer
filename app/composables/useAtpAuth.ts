  ...
  initPromise = (async () => {
    try {
      ...
      if (result) {
        ...
        console.log('[ATP] Session restored:', result.session.did);
      } else {
        agent.value = null;
      ...
      console.error('[ATP] OAuth initialization failed:', error);
...
export const useAtpAuth = () => {
  const login = async (handleOrPds: string) => {
    ...
    try {
      ...
      console.error('[ATP] Login failed:', error);
  ...
  const logout = async () => {
    ...
    try {
      ...
      console.error('[ATP] Logout failed:', error);
    } finally {
      agent.value = null;
      isAuthenticated.value = false;
    }
